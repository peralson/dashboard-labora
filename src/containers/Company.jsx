    ...
    <SelectedCompany.Provider value={{ value, setValue }}>
        ...
        {!loading && !error && (
          <Box mt={4}>
            ...
            <BlockContainer title={"General"} desc={"Información general utilizada para dar tramite a los procesos legales como la generación de contratos."}>
              <LegalPicker name={"Datos de contacto"} desc={"Email y teléfono"} />
            ...
            <BlockContainer
              ...
              cta={"Crear nuevo"}
              onClick={() => {}}
              desc={"Autoriza personal de tu empresa a gestionar los procesos de creación de ofertas y contratación de personal."}
            >
            ...
            <BlockContainer
              ...
              cta={"Editar"}
              onClick={() => {}}
              desc={"Categorías habilitadas la empresa para el desarrollo de sus actividades."}
            >
              <Grid templateColumns={"1fr 1fr 1fr 1fr"} w={"100%"} gap={4}>
                {company.data.categories.map((category, index) => (
                  <Flex
                    key={index}
                    alignItems={"center"}
      ...
      <Side>
        <SideSticky>
          <Documentation />
          <SideBoxContainer>
            {!value && <BeCurious text={"Selecciona alguna oferta de este proyecto"} />}
            {value === "Datos de empresa" && <LegalCompanySide legalData={company.data.legalData} />}
            {value === "Representante legal" && <LegalRepSide repData={company.data.repData} />}
            {value === "Datos de contacto" && <ContactSide contact={company.data.contact} />}
          </SideBoxContainer>
        </SideSticky>
      </Side>
